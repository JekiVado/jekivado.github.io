...
function pointerChartX(event, svg, frame) {
  const bounds = svg.getBoundingClientRect();
  const svgX = (event.clientX - bounds.left) / Math.max(bounds.width, 1) * frame.width;
  return Math.min(frame.pad.left + frame.innerWidth, Math.max(frame.pad.left, svgX));
}

function tooltipIndex(event, svg, frame, itemCount) {
  const progress = (pointerChartX(event, svg, frame) - frame.pad.left) / frame.innerWidth;
  return Math.round(progress * Math.max(0, itemCount - 1));
}

function barTooltipIndex(event, svg, frame, itemCount) {
  const svgX = pointerChartX(event, svg, frame);
...
function attachBarTooltip(svg, values, labels, frame) {
  ...
  svg.onpointermove = (event) => {
    ...
    const x = pointerChartX(event, svg, frame);
    guide.setAttribute('x1', x); guide.setAttribute('x2', x); guide.setAttribute('visibility', 'visible');
