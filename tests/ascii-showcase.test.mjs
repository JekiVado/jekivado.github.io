...
assert.match(page, /The Sanity<br \/>setup agents<br \/>don't reinvent\./);
...
assert.match(app, /function buildConcentricRings/);
assert.match(app, /const ringCount = 38;/);
assert.match(app, /const ringGap = 8;/);
assert.match(app, /const ringTrack = 'THE CONTENT ARCHITECTURE · ';/);
assert.match(app, /isText: ring % 3 === 0 \|\| ring > 31,/);
assert.match(app, /const fieldFrameInterval = 42;/);
assert.match(app, /return buildConcentricRings\(\);/);
assert.match(app, /function drawConcentricTextRings/);
assert.match(app, /const segmentAdvance = Math\.max\(18, fontSize \* ring\.track\.length \* \.56\);/);
assert.match(app, /context\.rotate\(angle \+ Math\.PI \/ 2\);/);
